...
This document provides guidance on testing the FxMap transport layer implementations (RabbitMQ, NATS, Kafka, gRPC, Azure Service Bus).
...
2. **Complex API Design**: Transport configuration uses a chained API (`FxMapRegister` -> `FxMapRegisterWrapped`) that's difficult to mock
...
### Unit Tests (Included)
- ✅ Core FxMap functionality (builders, helpers, accessors)
...
1. Create separate test project: `FxMap.IntegrationTests`
...
Transport layer testing is valuable but requires significant infrastructure setup. The current test suite focuses on core FxMap functionality that can be tested in isolation. For production use, implement the integration test strategy outlined above.
