...
FxMap 8.3.0 brings **full .NET 10 support** and modernizes the development experience with the new XML-based solution format (.slnx). This release ensures FxMap stays current with the latest .NET platform while maintaining backward compatibility with .NET 8 and .NET 9.
...
FxMap now fully supports **.NET 10**, the latest version of the .NET platform:
...
- **Dual detection strategy** for FxMap attributes
...
FxMap libraries now support three .NET versions simultaneously:
...
**Supported Packages:**
- `FxMap` - Core library
- `FxMap.Grpc` - gRPC transport
- `FxMap.Kafka` - Kafka transport
- `FxMap.Nats` - NATS transport
- `FxMap.RabbitMq` - RabbitMQ transport
- `FxMap.Azure.ServiceBus` - Azure Service Bus transport
- `FxMap.Aws.Sqs` - AWS SQS transport
- `FxMap.EntityFrameworkCore` - Entity Framework Core integration
- `FxMap.MongoDb` - MongoDB integration
- `FxMap.HotChocolate` - HotChocolate GraphQL integration
- `FxMap.Analyzers` - Roslyn analyzers
...
Project("{FAE04EC0-301F-11D3-BF4B-00C04F79EFBC}") = "FxMap", "src\FxMap\FxMap.csproj", "{CAB8CF6C-4A1D-43C6-836A-C5A7349D062E}"
EndProject
Global
...
**Problem:** Version conflicts between testing framework and FxMap assembly
...
```csharp
// Mock FxMapAttribute in test code
...
**Results:**
- All analyzer tests pass without FxMap assembly reference
...
While FxMap 8.3.0 doesn't add new features, running on .NET 10 provides:
...
dotnet add package FxMap.RabbitMq --version 8.3.0
# ... etc
```
...
- **FxMap.Core:** Core library (`FxMap`)
- **FxMap.Transports:** Message transport implementations
  - gRPC, Kafka, NATS, RabbitMQ, Azure Service Bus, AWS SQS
- **FxMap.DataProviders:** Data access integrations
  - Entity Framework Core, MongoDB
- **FxMap.Integrations:** Third-party integrations
  - HotChocolate (GraphQL)
- **FxMap.Analyzers:** Roslyn analyzers and tests
- **FxMap.Tests:** Unit tests and benchmarks
- **FxMap.Demo:** Sample applications
...
- **GitHub Issues:** https://github.com/quyvu01/FxMap/issues
- **Discussions:** https://github.com/quyvu01/FxMap/discussions
- **Documentation:** https://github.com/quyvu01/FxMap
...
FxMap 8.3.0 is a **forward-looking release** that:
...
5. **Share feedback** with the FxMap community

---

**FxMap 8.3.0** - Full .NET 10 Support & Modern Solution Format
...
Thank you for using FxMap! We're committed to delivering a high-performance, reliable microservices framework that stays current with the .NET ecosystem.

Your feedback and contributions help make FxMap better. We look forward to seeing what you build with FxMap 8.3.0!
