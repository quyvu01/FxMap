...
FxMap 8.0.25 introduces **production-grade observability and telemetry** across the entire framework. This release adds comprehensive OpenTelemetry integration, distributed tracing, metrics collection, and diagnostic events to all transport layers and database integrations.
...
This release transforms FxMap into a fully observable distributed system framework with:
...
**Request Metrics:**
```
fxmap.request.duration (Histogram)
  Labels: fxmap.attribute, fxmap.transport, fxmap.status

fxmap.active_requests (ObservableGauge)
  Real-time concurrent request count

fxmap.items.returned (Counter)
  Labels: fxmap.attribute, fxmap.transport

fxmap.errors.count (Counter)
  Labels: fxmap.attribute, fxmap.transport, fxmap.error_type
```

**Database Metrics:**
```
fxmap.database.query.duration (Histogram)
  Labels: fxmap.attribute, fxmap.db_system, fxmap.status

fxmap.database.errors.count (Counter)
  Labels: fxmap.attribute, fxmap.db_system, fxmap.error_type
```

### Diagnostic Events

**Request Lifecycle:**
- `fxmap.request.start` - Request initiated with context
- `fxmap.request.stop` - Request completed with metrics
- `fxmap.request.error` - Request failed with exception

**Database Lifecycle:**
- `fxmap.database.query.start` - Query started
- `fxmap.database.query.stop` - Query completed
- `fxmap.database.query.error` - Query failed

**Messaging Events:**
- `fxmap.message.receive` - Message received from broker
- `fxmap.message.send` - Message sent to broker
...
FxMap follows OpenTelemetry semantic conventions for maximum compatibility:
...
**FxMap-Specific Attributes:**
- `fxmap.attribute` - FxMap attribute type name
- `fxmap.transport` - Transport layer identifier
- `fxmap.expression` - Query expression string
- `fxmap.selector_ids` - Selector identifiers array
- `fxmap.item_count` - Number of items returned
- `fxmap.version` - FxMap framework version
...
builder.Services.AddOpenTelemetry()
    .WithTracing(tracing => tracing
        .AddSource("FxMap")  // FxMap telemetry
...
1. Update the package:
   ```bash
   dotnet add package FxMap --version 8.0.25
   ```

2. (Optional) Add OpenTelemetry configuration to enable telemetry:
   ```csharp
   builder.Services.AddOpenTelemetry()
       .WithTracing(t => t.AddSource("FxMap"))
       .WithMetrics(m => m.AddMeter("FxMap"));
   ```
...
- [FxMap Documentation](https://fxmapmapper.net)
...
1. Select "FxMap" service
2. Find your operation (e.g., "fxmap.request")
...
rate(fxmap_request_duration_sum[5m]) / rate(fxmap_request_duration_count[5m])

# Error rate
rate(fxmap_errors_count[5m])

# Active requests
fxmap_active_requests

# Database query duration p95
histogram_quantile(0.95, rate(fxmap_database_query_duration_bucket[5m]))
```
...
This release represents a major enhancement to FxMap's production readiness and enterprise capabilities. The comprehensive telemetry implementation enables teams to build, monitor, and maintain distributed systems with confidence.
...
**FxMap 8.0.25** - Production-Ready Distributed Observability
