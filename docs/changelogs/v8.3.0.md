...
- **All Projects Updated to Support .NET 10**
  ...
  - 19 projects updated across the entire FxMap ecosystem
  - Backward compatible with .NET 8 and .NET 9

**Affected Projects:**
- Core: `FxMap.csproj`
- Transports: `FxMap.Grpc`, `FxMap.Kafka`, `FxMap.Nats`, `FxMap.RabbitMq`, `FxMap.Azure.ServiceBus`, `FxMap.Aws.Sqs`
- Data Providers: `FxMap.EntityFrameworkCore`, `FxMap.MongoDb`
- Integrations: `FxMap.HotChocolate`
- Analyzers: `FxMap.Analyzers`, `FxMap.Analyzers.Tests`
- Tests: `FxMap.Tests`, `FxMap.Benchmark`
...
```xml
<Solution>
  <Folder Name="/FxMap.Analyzers/">
    <Project Path="src/FxMap.Analyzers/FxMap.Analyzers.csproj" />
    <Project Path="test/FxMap.Analyzers.Tests/FxMap.Analyzers.Tests.csproj" />
  </Folder>
  <Folder Name="/FxMap.Core/">
    <Project Path="src/FxMap/FxMap.csproj" />
  </Folder>
  <Folder Name="/FxMap.DataProviders/">
    <Project Path="src/FxMap.EntityFrameworkCore/FxMap.EntityFrameworkCore.csproj" />
    <Project Path="src/FxMap.MongoDb/FxMap.MongoDb.csproj" />
  </Folder>
  <Folder Name="/FxMap.Transports/">
    <Project Path="src/FxMap.Aws.Sqs/FxMap.Aws.Sqs.csproj" />
    <Project Path="src/FxMap.Azure.ServiceBus/FxMap.Azure.ServiceBus.csproj" />
    <Project Path="src/FxMap.Grpc/FxMap.Grpc.csproj" />
    <Project Path="src/FxMap.Kafka/FxMap.Kafka.csproj" />
    <Project Path="src/FxMap.Nats/FxMap.Nats.csproj" />
    <Project Path="src/FxMap.RabbitMq/FxMap.RabbitMq.csproj" />
  </Folder>
  <Folder Name="/FxMap.Integrations/">
    <Project Path="src/FxMap.HotChocolate/FxMap.HotChocolate.csproj" />
  </Folder>
  <Folder Name="/FxMap.Tests/">
    <Project Path="test/FxMap.Benchmark/FxMap.Benchmark.csproj" />
    <Project Path="test/FxMap.Tests/FxMap.Tests.csproj" />
  </Folder>
  <Folder Name="/FxMap.Demo/">
    ...
    <Folder Name="/FxMap.Demo/Contracts/">
...
- **Modified: `src/FxMap.Analyzers/FxMapExpressionSyntaxAnalyzer.cs`**
  - Added naming convention fallback for attribute detection
  - Improved `IsFxMapAttribute` method with dual detection strategy:
    1. Semantic analysis (preferred): Checks inheritance from `FxMap.Attributes.FxMapAttribute`
    2. Naming convention (fallback): Checks if attribute name ends with `OfAttribute`
  - Enables testing without FxMap assembly reference
...
- **Updated: `test/FxMap.Analyzers.Tests/FxMapExpressionSyntaxAnalyzerTests.cs`**
  - All test attributes now define mock `FxMapAttribute` in `FxMap.Attributes` namespace
  - Uses primary constructor syntax: `FxMapAttribute(string propertyName)`
  - Test attributes inherit from mock `FxMapAttribute` for semantic analysis validation
  - No dependency on FxMap assembly to avoid version conflicts
...
- All tests validate against proper `FxMapAttribute` inheritance
- Zero dependency on FxMap assembly in test project
...
- **Updated: All project versions from 8.2.6 to 8.3.0**
  - Core library: `src/FxMap/FxMap.csproj`
...
dotnet build FxMap.slnx
dotnet test FxMap.slnx
```
...
- **FxMap Requirement:** FxMap requires System.Runtime 8.0.0.0/9.0.0.0
- **Impact:** Cannot reference FxMap assembly in analyzer tests
...
3. **Mock Attributes:** Tests define lightweight `FxMapAttribute` mock
...
|-----------|----------------|--------|
| **Solution** | `FxMap.slnx` | Added |
| **Analyzer** | `src/FxMap.Analyzers/FxMapExpressionSyntaxAnalyzer.cs` | Enhanced |
| **Analyzer Tests** | `test/FxMap.Analyzers.Tests/FxMapExpressionSyntaxAnalyzerTests.cs` | Refactored |
...
1. **Update Package References:**
   ```bash
   dotnet add package FxMap --version 8.3.0
   # Update all FxMap ecosystem packages
   dotnet add package FxMap-gRPC --version 8.3.0
   dotnet add package FxMap-Kafka --version 8.3.0
   # ... etc
   ```
...
**FxMap 8.3.0** - .NET 10 Support & Modern Solution Format
