    ...
    : IRequestClient, IAsyncDisposable
{
        ...
        try
        {
            ...
            if (activity != null)
            {
                if (!string.IsNullOrEmpty(activity.Id))
                    messageAttributes.Add(SqsConstants.MessageAttributeTraceparent,
                        new MessageAttributeValue { DataType = "String", StringValue = activity.Id });
                if (!string.IsNullOrEmpty(activity.TraceStateString))
                    messageAttributes.Add(SqsConstants.MessageAttributeTracestate,
    ...
    private async Task InitializeAsync(CancellationToken cancellationToken)
    {
        ...
        var createQueueResponse = await _sqsClient.CreateQueueAsync(new CreateQueueRequest
        {
            ...
            {
                {
                    SqsConstants.AttributeMessageRetentionPeriod,
                    SqsConstants.DefaultMessageRetentionPeriod.ToString()
                },
                { SqsConstants.AttributeVisibilityTimeout, SqsConstants.DefaultVisibilityTimeout.ToString() },
                {
        ...
        while (!ct.IsCancellationRequested)
        {
            try
            {
                ...
                var processingResults = response.Messages.Select(message =>
                {
                    ...
                    {
                        // Get correlation ID
                        if (!message.MessageAttributes.TryGetValue(SqsConstants.MessageAttributeCorrelationId,
