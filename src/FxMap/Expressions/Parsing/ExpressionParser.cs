...
public sealed class ExpressionParser(IReadOnlyList<Token> tokens)
{
    ...
    private ExpressionNode ParseSegmentWithoutTrailingFunction()
    {
        ...
        // Validate: next segment requires '.' before identifier
        if (Check(TokenType.Identifier))
        {
            throw new ExpressionParseException($"Property navigation requires '.' before identifier at position {Current().Position}");
        }
    ...
    private ExpressionNode ParseNavigationExpression()
    {
        ...
        // Validate: projection must have '.' before '{'
        if (Check(TokenType.OpenBrace))
        {
            throw new ExpressionParseException($"Projection requires '.' before '{{' at position {Current().Position}");
        }
