...
namespace FxMap.Telemetry;

/// <summary>
/// Provides OpenTelemetry-compatible distributed tracing support for FxMap framework.
/// </summary>
/// <remarks>
...
/// </remarks>
public static class FxMapActivitySource
{
    ...
    /// <remarks>
    /// Client activities represent outbound requests from the application to FxMap servers.
    ...
    /// <summary>
    /// Starts an internal activity for FxMap operations (e.g., expression parsing, projection).
    /// </summary>
    ...
    public static Activity StartDatabaseActivity<TDistributedKey>(string dbSystem, string operationName = null)
        where TDistributedKey : IDistributedKey
    {
        var activity = Source.StartActivity(operationName ?? "fxmap.db.query", ActivityKind.Client);
    ...
    extension(Activity activity)
    {
        ...
        /// <summary>
        /// Adds FxMap-specific tags to an activity.
        /// </summary>
        /// <param name="expressions">The FxMap expressions being evaluated.</param>
        /// <param name="selectorIds">The selector IDs being queried.</param>
        /// <param name="itemCount">The number of items returned.</param>
        public void SetFxMapTags(string[] expressions = null, string[] selectorIds = null, int? itemCount = null)
        {
            if (activity is null) return;

            activity.SetTag(Constants.Telemetry.TagFxMapExpressions, expressions);
            if (selectorIds is { Length: > 0 })
            {
                activity.SetTag(Constants.Telemetry.TagFxMapSelectorCount, selectorIds.Length);
                // Only log first few IDs to avoid huge tags
                activity.SetTag(Constants.Telemetry.TagFxMapSelectorIds, string.Join(",", selectorIds.Take(5)));
            }

            if (itemCount.HasValue) activity.SetTag(Constants.Telemetry.TagFxMapItemCount, itemCount.Value);
        }
    }
}